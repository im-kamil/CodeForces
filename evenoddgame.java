...
public class evenoddgame {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(t-->0){
            int n=sc.nextInt();
            PriorityQueue<Integer> even,odd;
            ...
            while(even.size()>0 || odd.size()>0){
                if(z){
                    if(even.size()==0) odd.remove();
                    else if(odd.size()==0) alice+=even.remove();
                    else if(even.peek()>odd.peek())alice+=even.remove();
                    else odd.remove();
                    z=false;
                }
                else{
                    if(even.size()==0) bob+=odd.remove();
                    else if(odd.size()==0) even.remove();
                    else if(odd.peek()>even.peek())bob+=odd.remove();
                    else even.remove();
                    z=true;
                }
            }
            if(alice>bob) System.out.println("Alice");
            else if(bob>alice) System.out.println("Bob");
